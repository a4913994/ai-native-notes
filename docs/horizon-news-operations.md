...
标题展示：英文标题在排序和重点摘要完成后，由同一个 DeepSeek 服务每批最多 20 条翻译成简体中文；已有中文标题保持不变。前 20 条正文标题和目录使用同一译名，其余折叠列表也使用中文标题，原文链接保持不变并在新标签页打开。翻译结果按固定序号校验，缺项、重复序号、错误格式或超时重试一次；仍失败时保留对应原标题并显示翻译不可用提示，不删除资讯。译文随日报 artifact 保存，补传不重复调用模型。
