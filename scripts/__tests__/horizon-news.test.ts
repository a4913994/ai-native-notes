...
describe('Horizon adapter failure and publication boundaries', () => {
  for (const scenario of ['ready', 'empty', 'partial', 'swallowed-failure', 'twitter-failure', 'twitter-security', 'extra-sources', 'all-headlines', 'title-translation', 'all-failed', 'ai-failed', 'enrichment-failed', 'old-file']) {
    it(scenario, async () => {
