...
- Live retry acceptance: run 35433332639 skipped generation for the existing day; run 35433335702 restored the saved artifact and republished successfully without AI calls. Same-generation upserts preserve publication/update timestamps. Local full suite: 486 passed, 0 failed; client/server/CLI checks passed. Production frontend deployment: `ef7e7bc8` (followed by presentation-only citation cleanup).
...
- 2026-09-19：资讯和个人文章加入常驻阅读导航、跳转后返回位置、外链新标签页及浏览器历史滚动／折叠恢复。资讯日期列表分页不再替换正文，使用 30 秒读取缓存；写作入口按需加载。496 项测试与前端构建通过，详见 `docs/reading-experience.md`。
